...
export default function PickupCalculator({ supabase }) {
  ...
  function rankResults(results, query) {
    ...
    const scored = results.map((item) => {
      ...
      const goodTypes = ["city", "town", "village", "state", "county", "administrative"];
  ...
  function selectSuggestion(suggestion) {
    ...
    setSuggestions([]);
    setTimeout(() => handleCalculate(suggestion.display_name, {
      lat: parseFloat(suggestion.lat),
      lon: parseFloat(suggestion.lon),
    }), 100);
  }

  async function checkCache(loc) {
    const key = loc.toLowerCase().trim();

    // 1. Check in-memory cache first (instant)
    if (memoryCache.current[key]) {
      return { ...memoryCache.current[key], source: 'memory' };
    }
    ...
    if (supabase) {
      ...
      if (!error && data) {
        ...
        memoryCache.current[key] = { distance: data.distance, cost: data.cost };
        return { distance: data.distance, cost: data.cost, source: 'db' };
      }
    }
  ...
  async function saveToCache(loc, distance, cost) {
    const key = loc.toLowerCase().trim();

    // Save to memory cache
    memoryCache.current[key] = { distance, cost };

    // Save to Supabase
    if (supabase) {
      await supabase
        .from('location_cache')
        .upsert({ location: loc.trim(), distance, cost }, { onConflict: 'location' });
    }
  }

  async function geocodeLocation(loc) {
    const apiKey = "pk.ad8425665c12e1b7f5d7827258d59077";
    const url = `https://us1.locationiq.com/v1/search?key=${apiKey}&q=${encodeURIComponent(loc)}, USA&format=json&limit=1`;
    const response = await fetch(url);
    if (!response.ok) throw new Error("Failed to find location. Please try again.");
    const data = await response.json();
    if (data.length === 0) throw new Error("Location not found. Please check the address and try again.");
  ...
  async function getDrivingDistance(origin) {
    ...
    return Math.round(distanceMeters * 0.000621371 * 1.03);
  }

  async function handleCalculate(locOverride, coordsOverride) {
    const loc = (locOverride || location).trim();
    const coords = coordsOverride || selectedCoords;
    ...
    try {
      ...
      setResult({ from: loc, distance, cost });
    } catch (err) {
  ...
  return (
    <div className="fade-in">
      <style>{`
        ...
        .pickup-calc-autocomplete { position: relative; }
        ...
        .pickup-calc-suggestion:last-child { border-bottom: none; }
        .pickup-calc-suggestion:hover, .pickup-calc-suggestion.active {
          background-color: rgba(232, 180, 74, 0.1);
        }
        .pickup-calc-suggestion-name { color: var(--text); font-weight: 500; font-size: 14px; }
        .pickup-calc-suggestion-details { color: var(--muted); font-size: 12px; margin-top: 2px; }
        .pickup-calc-result {
          margin-top: 20px;
          padding: 20px;
        ...
        .pickup-calc-cache-badge {
          background: rgba(74, 232, 133, 0.2);
          color: var(--success);
          font-size: 10px;
          padding: 2px 7px;
          border-radius: 10px;
        ...
        .pickup-calc-result-details { color: var(--text); font-size: 14px; line-height: 1.6; }
      `}</style>
      ...
      <div className="form-card">
              ...
              <div className="pickup-calc-suggestions" ref={suggestionsRef}>
                {suggestions.map((item, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className={`pickup-calc-suggestion ${index === selectedSuggestionIndex ? "active" : ""}`}
                      onClick={() => selectSuggestion(item)}
                    >
                      <div className="pickup-calc-suggestion-name">{primaryText}</div>
                      {secondaryText && (
                        <div className="pickup-calc-suggestion-details">{secondaryText}</div>
                      )}
                    </div>
                  );
                })}
        ...
        {result && (
          <div className="pickup-calc-result">
            <div className="pickup-calc-result-label">
              Total Travel Cost
              {fromCache && <span className="pickup-calc-cache-badge">CACHED</span>}
            </div>
            <div className="pickup-calc-result-value">${result.cost}</div>
            <div className="pickup-calc-result-details">
              <strong>Distance:</strong> {result.distance.toLocaleString()} miles<br />
